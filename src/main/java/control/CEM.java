...
public class CEM {
	...
	private static final String SENHA = System.getenv("MAIL_PASSWORD");
	...
	public static void resgateSenha(TicketSenha ticket) {
		...
		try {
			...
			link = System.getenv("DEFAULT_URL") + "resetsenha.jsp?code="
					+ URLEncoder.encode(ticket.getTicketPass(), "UTF-8");
		} catch (Exception ex) {
			ex.printStackTrace();
			return;
		}
		htmlMsg = htmlMsg.replaceAll("%link%", Matcher.quoteReplacement(link));
		htmlMsg = htmlMsg.replaceAll("%url%", System.getenv("DEFAULT_URL"));
